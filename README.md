...
## **17. Проектирование реляционных схем: аномалии, декомпозиция схем-отношений, нормальная форма Бойса-Кодда, декомпозиция в BCNF, третья нормальная форма.**

### **Проектирование реляционных схем: нормальные формы и декомпозиция**  

Реляционные базы данных требуют правильного проектирования, чтобы избежать **аномалий** (ошибок при вставке, обновлении и удалении данных). Для этого используются **нормальные формы** (1NF, 2NF, 3NF, BCNF), которые помогают устранить избыточность и обеспечить целостность данных.

---

### **1. Аномалии в реляционных базах данных**  
Аномалии возникают из-за неправильной структуры таблиц.  

#### **1.1. Типы аномалий**  
| Аномалия | Описание | Пример |
|----------|----------|--------|
| **Вставки** | Невозможность добавить данные без нарушения связей | Нельзя добавить студента без группы, если `Группа` — обязательное поле. |
| **Обновления** | Необходимость изменять одни и те же данные в нескольких местах | Если преподаватель меняет фамилию, её нужно обновить во всех связанных курсах. |
| **Удаления** | Потеря данных при удалении связанных записей | Удаление курса может стереть информацию о студентах, если нет каскадного удаления. |

**Пример плохой схемы**:  
```sql
CREATE TABLE Записи (
    Студент_ID INT,
    Студент_Имя VARCHAR(50),
    Курс_ID INT,
    Курс_Название VARCHAR(100),
    Преподаватель VARCHAR(50)
);
```
**Проблемы**:  
- Избыточность (имя студента дублируется).  
- Аномалии обновления (если студент меняет имя, нужно обновить все записи).  

---

### **2. Декомпозиция схем отношений**  
**Декомпозиция** — разбиение одной таблицы на несколько для устранения аномалий.  

**Требования к декомпозиции**:  
1. **Без потерь (Lossless Join)** – после соединения таблиц должны получаться исходные данные.  
2. **Сохранение зависимостей** – все функциональные зависимости должны сохраняться.  

**Пример декомпозиции**:  
Исходная таблица `Записи` разбивается на:  
- `Студенты`  
- `Курсы`  
- `Записи_на_курсы` (связь M:N).  
...
- Должна выполняться **1NF**.  
- Все неключевые атрибуты зависят **от всего первичного ключа** (а не от его части).  

**Пример нарушения 2NF**:  
| **Студент_ID** (PK) | **Курс_ID** (PK) | **Студент_Имя** | **Курс_Название** |  
|---------------------|------------------|-----------------|-------------------|  
| 1 | 101 | Иван | Математика |  
| 1 | 102 | Иван | Физика |  

**Проблема**:  
- `Студент_Имя` зависит только от `Студент_ID`, а не от всей пары `(Студент_ID, Курс_ID)`.  

**Исправление**:  
Разделить на две таблицы:  
1. `Студенты(Студент_ID, Студент_Имя)`  
2. `Записи_на_курсы(Студент_ID, Курс_ID, Курс_Название)`  

---

#### **3.3. Третья нормальная форма (3NF)**  
- Должна выполняться **2NF**.  
- Нет **транзитивных зависимостей** (если `A → B → C`, то `B → C` выносится в отдельную таблицу).  

**Пример нарушения 3NF**:  
| **Студент_ID** | **Группа** | **Факультет** |  
|----------------|------------|---------------|  
| 1 | 101 | ФИЗИКА |  
| 2 | 101 | ФИЗИКА |  

**Проблема**:  
- `Факультет` зависит от `Группа`, а не напрямую от `Студент_ID`.  

**Исправление**:  
1. `Студенты(Студент_ID, Группа)`  
2. `Группы(Группа, Факультет)`  

---

#### **3.4. Нормальная форма Бойса-Кодда (BCNF)**  
- Усиленная версия **3NF**.  
- Требует, чтобы **каждая детерминанта (левая часть зависимости) была потенциальным ключом**.  
...
1. `(Студент, Курс) → Преподаватель`  
2. `Преподаватель → Курс`  

**Проблема**:  
- `Преподаватель` не является ключом, но определяет `Курс`.  

**Исправление**:  
1. `Преподаватели(Преподаватель, Курс)`  
2. `Записи(Студент, Преподаватель)`  

---

### **4. Сравнение нормальных форм**  
| Нормальная форма | Условие | Решаемые проблемы |  
|------------------|---------|-------------------|  
| **1NF** | Атомарные атрибуты | Устранение дублирования в ячейках |  
| **2NF** | Зависимость от всего PK | Устранение частичных зависимостей |  
| **3NF** | Нет транзитивных зависимостей | Устранение косвенных зависимостей |  
| **BCNF** | Детерминанты — потенциальные ключи | Устранение всех функциональных зависимостей, не связанных с ключами |  

---

### **5. Пример полной декомпозиции в BCNF**  
**Исходная таблица (нарушает BCNF)**:  
| **Заказ_ID** | **Товар** | **Категория_товара** |  
|--------------|-----------|----------------------|  
| 1 | Ноутбук | Электроника |  
| 1 | Книга | Канцелярия |  

**Проблема**:  
- `Категория_товара` зависит от `Товар`, но `Товар` не является ключом.  

**Декомпозиция**:  
1. `Заказы(Заказ_ID, Товар)`  
2. `Товары(Товар, Категория_товара)`  

---

### **Вывод**  
- **Нормальные формы** помогают избежать аномалий и избыточности.  
- **1NF → 2NF → 3NF → BCNF** – последовательное улучшение структуры.  
- **BCNF** – наиболее строгая форма, устраняющая все зависимости, не связанные с ключами.  
- Декомпозиция должна быть **без потерь** и **сохранять зависимости**.  

**Практическое правило**:  
- В большинстве случаев достаточно **3NF**.  
- **BCNF** применяется, когда есть сложные функциональные зависимости.

---

## **18. Алгебра реляционных операций: основы реляционной алгебры, операции над мультимножествами, дополнительные операции. Алгебра как язык описания ограничений.**

### **Алгебра реляционных операций**  

Реляционная алгебра — это формальный язык операций над таблицами (отношениями), который лежит в основе SQL и реляционных СУБД. Она позволяет описывать запросы и ограничения на данных.  

---

### **1. Основы реляционной алгебры**  
Реляционная алгебра работает с **отношениями** (таблицами) и поддерживает операции:  

#### **1.1. Базовые операции**  
| Операция | Обозначение | SQL-аналог | Описание |
|----------|-------------|------------|----------|
| **Выборка (σ)** | `σ(условие)(R)` | `SELECT * FROM R WHERE условие` | Фильтрация строк |
| **Проекция (π)** | `π(A₁, A₂)(R)` | `SELECT A₁, A₂ FROM R` | Выбор столбцов |
| **Объединение (∪)** | `R ∪ S` | `UNION` | Все уникальные строки из `R` и `S` |
| **Разность (−)** | `R − S` | `EXCEPT` | Строки из `R`, которых нет в `S` |
| **Декартово произведение (×)** | `R × S` | `CROSS JOIN` | Все комбинации строк `R` и `S` |
| **Переименование (ρ)** | `ρ(new_name)(R)` | `AS` | Изменение имени таблицы/атрибута |

**Пример**:  
```sql
-- SQL
SELECT Имя, Фамилия FROM Студент WHERE Группа = 101;

-- Реляционная алгебра
π(Имя, Фамилия)(σ(Группа=101)(Студент))
```

---

#### **1.2. Соединения (Join-операции)**  
| Операция | Обозначение | SQL-аналог | Описание |
|----------|-------------|------------|----------|
| **Естественное соединение (⋈)** | `R ⋈ S` | `NATURAL JOIN` | Соединение по общим атрибутам |
| **Тета-соединение (⋈ᶿ)** | `R ⋈(AθB) S` | `JOIN ON AθB` | Соединение с условием `θ` (`=`, `<`, `>`) |
| **Полусоединение (⋉, ⋊)** | `R ⋉ S` | `EXISTS` | Только строки `R`, имеющие соответствие в `S` |

**Пример**:  
```sql
-- SQL
SELECT * FROM Студент JOIN Группа ON Студент.Группа_ID = Группа.ID;

-- Реляционная алгебра
Студент ⋈(Группа_ID=ID) Группа
```

---

### **2. Операции над мультимножествами (SQL-style)**  
В отличие от классической алгебры, SQL работает с **мультимножествами** (дубликаты разрешены).  

#### **2.1. Мультимножественные аналоги**  
| Операция | SQL-аналог | Описание |
|----------|------------|----------|
| **Мультимножественное объединение** | `UNION ALL` | Объединение с дубликатами |
| **Мультимножественное пересечение** | `INTERSECT ALL` | Пересечение с учётом кратности |
| **Мультимножественная разность** | `EXCEPT ALL` | Разность с учётом кратности |

**Пример**:  
```sql
-- SQL (мультимножественное объединение)
SELECT * FROM R UNION ALL SELECT * FROM S;

-- Реляционная алгебра (нет стандартного обозначения, обычно используют ⋃⁺)
```
...
#### **3.1. Агрегация (γ)**  
Группировка + вычисление агрегатных функций (`SUM`, `AVG`, `COUNT`).  
```sql
-- SQL
SELECT Группа, COUNT(*) FROM Студент GROUP BY Группа;

-- Реляционная алгебра
γ(Группа, COUNT(*))(Студент)
```

#### **3.2. Деление (÷)**  
Находит кортежи в `R`, связанные **со всеми** кортежами из `S`.  
```sql
-- SQL (через NOT EXISTS)
SELECT DISTINCT A FROM R r1
WHERE NOT EXISTS (
    SELECT * FROM S WHERE NOT EXISTS (
        SELECT * FROM R r2 WHERE r2.A = r1.A AND r2.B = S.B
    )
);
...
### **4. Алгебра как язык описания ограничений**  
Реляционная алгебра может выражать **ограничения целостности** (constraints):  

#### **4.1. Проверка ключей**  
Первичный ключ должен быть уникален:  
```
π(K)(R) = π(K)(γ(K, COUNT(*))(R))  -- COUNT(*) = 1 для всех K
```

#### **4.2. Внешние ключи**  
Каждое значение FK должно существовать в PK:  
```
π(FK)(R) ⊆ π(PK)(S)
```

#### **4.3. Условные ограничения**  
Например, зарплата менеджера ≥ 1000:  
```
σ(Должность='Менеджер' AND Зарплата < 1000)(R) = ∅
```

---

### **5. Пример комплексного запроса**  
**Задача**: Найти студентов, записанных на все курсы преподавателя "Иванов".  

**Реляционная алгебра**:  
1. Найти курсы Иванова:  
   `Курсы_Иванова = π(Курс_ID)(σ(Преподаватель='Иванов')(Преподаватель_Курс))`  
2. Разделить записи на курсы:  
   `Студенты_на_всех = Записи ÷ Курсы_Иванова`  
3. Получить имена студентов:  
   `π(Имя, Фамилия)(Студенты ⋈ Студенты_на_всех)`  

**SQL-эквивалент**:  
```sql
SELECT Имя, Фамилия FROM Студент
WHERE NOT EXISTS (
    SELECT * FROM Преподаватель_Курс
    WHERE Преподаватель = 'Иванов'
    AND NOT EXISTS (
        SELECT * FROM Записи
        WHERE Записи.Студент_ID = Студент.ID
        AND Записи.Курс_ID = Преподаватель_Курс.Курс_ID
    )
);
```
...
### **Вывод**  
- Реляционная алгебра — **формальная основа SQL**.  
- **Базовые операции**: выборка, проекция, соединения, объединение.  
- **Мультимножественные операции** учитывают дубликаты (как в SQL).  
- **Дополнительные операции**: агрегация, деление.  
- **Ограничения целостности** можно выразить через алгебру.  

**Практическое применение**:  
- Оптимизация запросов (СУБД преобразуют SQL в алгебру).  
- Проверка корректности схемы БД.  
- Теоретическое обоснование реляционных операций.

---

## **19. Параллельные вычисления. Многопроцессорность и многопоточность. Переключение контекста. Приоритеты потоков. Поточно-локальные данные (thread local storage).**

### **Параллельные вычисления: многопроцессорность и многопоточность**  

Параллельные вычисления позволяют выполнять несколько задач одновременно, используя:  
- **Многопроцессорность** – несколько процессов (изолированных, с отдельной памятью).  
- **Многопоточность** – несколько потоков в одном процессе (общая память, но свои стеки).  

---

### **1. Многопроцессорность vs Многопоточность**  

| Критерий          | Многопроцессорность               | Многопоточность                  |
|-------------------|-----------------------------------|----------------------------------|
| **Память**        | Изолированная (межпроцессное взаимодействие сложнее) | Общая (легко обмениваться данными, но есть race conditions) |
| **Создание**      | Тяжелее (нужна отдельная копия памяти) | Легче (потоки дешевле процессов) |
| **Отказоустойчивость** | Если падает один процесс, другие работают | Если падает поток, может упасть весь процесс |
| **Примеры**       | `fork()` в Unix, Python `multiprocessing` | Java `Thread`, C++ `std::thread` |

**Когда использовать**:  
- **Многопроцессорность**: Если нужна изоляция (например, веб-сервер с процессами).  
- **Многопоточность**: Если задачи часто обмениваются данными (например, обработка изображений).  
...
**Что это**:  
Когда ОС переключает CPU с одного потока/процесса на другой, сохраняя состояние (регистры, стек, PC).  

**Типы**:  
- **Переключение потоков** (быстрее, т.к. память общая).  
- **Переключение процессов** (медленнее, т.к. нужно обновлять таблицы памяти).  

**Проблемы**:  
- **Накладные расходы** – CPU тратит время на сохранение/восстановление состояния.  
- **Кэш-промахи** – после переключения кэш CPU может быть неактуальным.  

**Как уменьшить**:  
- Использовать **асинхронные модели** (например, корутины в Python).  
- Увеличить **квант времени** потока (но это снижает отзывчивость).  

---

### **3. Приоритеты потоков**  

ОС назначает потокам приоритеты, чтобы решить, какой поток выполнять первым.  

**Примеры приоритетов**:  
- **Высокий**: Реалтайм-задачи (например, аудиопоток).  
- **Нормальный**: Пользовательские приложения.  
- **Низкий**: Фоновые задачи (логирование).  

**Управление приоритетами**:  
- В Java: `thread.setPriority(Thread.MAX_PRIORITY)`.  
- В C++: `std::thread::native_handle()` + API ОС.  

**Проблемы**:  
- **Голодание** – если высокоприоритетные потоки всегда заняты, низкоприоритетные не выполняются.  
- **Инверсия приоритетов** – низкоприоритетный поток блокирует высокоприоритетный (решается **наследственным приоритетом**).  

---

### **4. Поточно-локальные данные (Thread-Local Storage, TLS)**  

**Что это**:  
Переменные, которые уникальны для каждого потока (даже если потоков много, у каждого своя копия).  

**Зачем нужно**:  
- Хранение состояния, которое не должно делиться между потоками (например, `errno` в C).  
- Избежание блокировок (не нужно синхронизировать доступ).  
...
```cpp
#include <thread>
thread_local int counter = 0;  // У каждого потока своя копия counter

void foo() {
    counter++;  // Безопасно, даже если foo() вызывают много потоков
}
```
...
def foo():
    thread_local.counter = getattr(thread_local, "counter", 0) + 1
```

**Плюсы**:  
- **Безопасность** – нет гонки данных (race conditions).  
- **Производительность** – не нужны блокировки.  

**Минусы**:  
- **Память** – каждый поток хранит свою копию.  
- **Сложность отладки** – трудно отслеживать значения в разных потоках.  

---

### **5. Вывод**  

| Параметр          | Многопроцессорность | Многопоточность | TLS |
|-------------------|---------------------|------------------|-----|
| **Память**        | Изолированная       | Общая            | Локальная для потока |
| **Скорость**      | Медленнее           | Быстрее          | Быстрый доступ |
| **Безопасность**  | Высокая             | Нужны блокировки | Полная |
| **Использование** | Изолированные задачи | Высокая связанность задач | Состояние потока |

**Рекомендации**:  
- Используйте **многопоточность**, если задачи часто обмениваются данными.  
- Используйте **многопроцессорность**, если важна изоляция (например, веб-серверы).  
- **TLS** – если нужно хранить состояние потока без блокировок.  

**Пример**:  
Веб-сервер (например, Nginx) использует **многопроцессорность** для изоляции запросов, но внутри каждого процесса — **многопоточность** для обработки соединений, а **TLS** хранит данные сессии.

---

## **20. Основные проблемы, возникающие при многопоточном программировании. Способы синхронизация. Синхронизационные примитивы: мьютексы, семафоры, события.**

### **Основные проблемы многопоточного программирования**  

При работе с потоками возникают следующие проблемы:  

1. **Гонка данных (Race Condition)**  
   - Несколько потоков одновременно изменяют общие данные, что приводит к неопределённому поведению.  
   - **Пример**: Два потока увеличивают одну переменную без синхронизации.  

2. **Взаимная блокировка (Deadlock)**  
   - Два или более потоков блокируют друг друга, ожидая освобождения ресурсов.  
   - **Пример**: Поток 1 держит `Мьютекс A` и ждёт `Мьютекс B`, а Поток 2 держит `Мьютекс B` и ждёт `Мьютекс A`.  

3. **Голодание (Starvation)**  
   - Поток не получает доступ к ресурсу, потому что другие потоки постоянно его занимают.  
   - **Пример**: Высокоприоритетные потоки всегда выполняются раньше низкоприоритетных.  

4. **Инверсия приоритетов (Priority Inversion)**  
   - Низкоприоритетный поток временно блокирует высокоприоритетный (например, удерживая мьютекс).  

5. **Состояние гонки на сигналах (Signal Handling Race)**  
   - Неправильная обработка сигналов в многопоточной среде может привести к падению.  

---

### **Способы синхронизации**  

Для решения этих проблем используются **синхронизационные примитивы**:  

#### **1. Мьютекс (Mutex, Mutual Exclusion)**  
- Гарантирует, что только **один поток** может выполнять критическую секцию кода.  
- **Блокирующий**: Если мьютекс занят, поток ждёт его освобождения.  
...
```
**Проблемы**:  
- **Deadlock**, если поток забудет `unlock()`.  
- **Используйте `std::lock_guard`** для автоматического освобождения:  
  ```cpp
  void safe_increment(int& counter) {
...
#### **2. Семафор (Semaphore)**  
- Расширенный счётчик, который ограничивает количество потоков в критической секции.  
- **Виды**:  
  - **Бинарный семафор** (аналог мьютекса, но может освобождаться другим потоком).  
  - **Счётный семафор** (например, для ограничения пула соединений).  

**Пример (C++20)**:  
```cpp
#include <semaphore>
std::counting_semaphore<10> sem(3);  // Макс. 3 потока одновременно
...
**Отличие от мьютекса**:  
- Мьютекс — только 1 поток, семафор — `N` потоков.  

---

#### **3. Условные переменные (Condition Variables)**  
- Позволяют потокам ждать **события** (например, изменения данных).  
- Всегда используются **с мьютексом**.  
...
void consumer() {
    ...
    cv.wait(lock, [] { return data_ready; });  // Ждёт, пока data_ready == true
    // Обработка данных...
}
```

**Применение**:  
- Очереди задач (поток ждёт новые задачи).  

---

#### **4. События (Events)**  
- Механизм уведомления потоков (аналог условных переменных, но без мьютекса).  
- **Пример (Windows API)**:  
  ```cpp
...
#### **5. Атомарные операции (Atomic)**  
- Операции, которые выполняются **за один такт CPU** (без блокировок).  
- **Пример (C++)**:  
  ```cpp
  #include <atomic>
  ...
  ```
**Плюсы**:  
- Быстрее мьютексов для простых операций (`++`, `load`, `store`).  

**Минусы**:  
- Не подходит для сложных критических секций.  
...
|-------------------|---------------------------------------|--------------------------------|----------------------------|
| **Мьютекс**       | Защита критической секции             | Простота                       | Риск deadlock              |
| **Семафор**       | Ограничение доступа к ресурсу (N потоков) | Гибкость                      | Сложнее отладки            |
| **Условная переменная** | Ожидание события                   | Эффективное ожидание           | Нужен мьютекс              |
| **Атомарные операции** | Простые операции (счётчики)      | Максимальная производительность | Ограниченная функциональность |

---

### **Вывод**  
1. **Гонки данных** решаются **мьютексами, атомарными операциями**.  
2. **Deadlock** избегают:  
   - Используя `std::lock_guard`.  
   - Упорядочивая захват мьютексов.  
3. **Голодание** предотвращают:  
   - Чередуя доступ (например, семафоры).  
   - Используя fair-мьютексы.  
4. **Условные переменные** и **события** помогают синхронизировать сложные сценарии.  

**Золотое правило**:  
> Минимизируйте время удержания блокировок и используйте атомарные операции там, где это возможно.  

**Пример**:  
Потокобезопасная очередь:  
- `push/pop` защищаются **мьютексом**.  
- Ожидание элементов — **условная переменная**.  
- Счётчик элементов — **атомарный**.

---

## **21. Виртуальная память, цели и реализация. Буфер быстрого преобразования адреса (TLB).**

### **Виртуальная память: цели и реализация**  

**Виртуальная память** — это механизм, который позволяет программам использовать **логические адреса**, не зависящие от физического расположения данных в RAM или на диске.  

---

### **1. Основные цели виртуальной памяти**  

1. **Изоляция процессов**  
   - Каждая программа "видит" свою линейную память (0x0000... – 0xFFFF...), не мешая другим.  
   - Пример: Процесс A не может случайно перезаписать данные процесса B.  

2. **Упрощение программирования**  
   - Программы работают с **непрерывными** адресными пространствами, даже если физическая память фрагментирована.  

3. **Защита памяти**  
   - Страницы могут быть помечены как **только для чтения/исполнения**, предотвращая ошибки (например, перезапись кода).  

4. **Расширение доступной памяти (своппинг)**  
   - Редко используемые данные перемещаются на **диск** (в swap-файл), освобождая RAM.  

5. **Поддержка отображения файлов в память (mmap)**  
   - Файлы могут быть загружены в виртуальную память, что ускоряет доступ (например, БД используют mmap).  

---

### **2. Реализация виртуальной памяти**  

#### **2.1. Страничная организация (Paging)**  
- Виртуальная память делится на **страницы** (обычно 4 КБ), физическая — на **фреймы**.  
- **Таблица страниц (Page Table)** хранит соответствие виртуальных адресов → физическим.  

##### **Структура таблицы страниц**  
| Виртуальный адрес | Физический адрес | Флаги (R/W/X) |  
|-------------------|------------------|---------------|  
| 0x0000 | 0x1000 | R W |  
| 0x1000 | 0x4000 | R X |  
| 0x2000 | **Отсутствует** (страница на диске) | — |  

- Если страницы нет в RAM (Page Fault), ОС загружает её с диска.  

#### **2.2. Сегментная организация (Segmentation)***  
*(Устарело, но используется в x86 для совместимости)*  
- Память делится на **сегменты** (код, данные, стек), каждый со своим базовым адресом и размером.  
- **Недостаток**: Фрагментация и сложность управления.  

---

### **3. Буфер быстрого преобразования адресов (TLB)**  

**TLB (Translation Lookaside Buffer)** — это **кэш** для таблицы страниц, ускоряющий преобразование виртуальных адресов в физические.  

#### **3.1. Как работает TLB**  
1. CPU генерирует **виртуальный адрес**.  
2. TLB проверяет, есть ли его перевод в кэше:  
   - **Попадание (TLB Hit)**: Физический адрес возвращается мгновенно.  
   - **Промах (TLB Miss)**: ОС ищет адрес в **таблице страниц**, затем обновляет TLB.  

#### **3.2. Типы TLB**  
- **Инструкционный (ITLB)**: Кэширует адреса команд.  
- **Данные (DTLB)**: Кэширует адреса данных.  
- **Уровни (L1, L2)**: Как в CPU-кэше (L1 быстрее, но меньше).  

#### **3.3. Оптимизации TLB**  
- **ASID (Address Space ID)**: Различает записи TLB для разных процессов.  
- **Большие страницы (HugePages)**: Уменьшают число TLB-записей (например, 2 МБ вместо 4 КБ).  

**Пример**:  
```
Витр. адрес: 0x12345678 → TLB → Физ. адрес: 0xABCD0000 (если есть в кэше).
```

---

### **4. Пример работы виртуальной памяти**  

1. **Процесс запрашивает доступ к 0xABCD1234**.  
2. **TLB проверяет кэш**:  
   - Если есть → возвращает физический адрес.  
   - Если нет → ОС ищет в **таблице страниц**.  
3. **Если страница отсутствует в RAM**:  
   - Происходит **Page Fault** → ОС загружает данные с диска.  
4. **TLB обновляется**, чтобы ускорить следующий доступ.  

---

### **5. Производительность и проблемы**  

#### **5.1. TLB-промахи**  
- Замедляют работу (обращение к таблице страниц в RAM занимает ~100 тактов CPU).  
- **Решение**:  
  - Увеличить размер TLB (но дорого).  
  - Использовать **HugePages** (меньше записей → меньше промахов).  

#### **5.2. Фрагментация таблицы страниц**  
- Таблицы могут занимать много памяти (у 64-битных систем — терабайты!).  
- **Решение**:  
  - **Многоуровневые таблицы страниц** (например, 4 уровня в x86-64).  
  - **Обратные таблицы (Inverted Page Tables)** — одна таблица на всю систему.  

#### **5.3. Overhead своппинга**  
- Частые Page Faults из-за нехватки RAM → тормоза.  
- **Решение**: Увеличить RAM или оптимизировать алгоритм подкачки.  
...
| Компонент          | Роль |  
|--------------------|------|  
| **Виртуальная память** | Изолирует процессы, обеспечивает защиту и своппинг. |  
| **Таблица страниц** | Хранит соответствие виртуальных → физических адресов. |  
| **TLB**            | Кэширует частые переводы адресов для скорости. |  

**Итог**:  
- Виртуальная память + TLB — ключевые механизмы для **безопасности** и **производительности**.  
- Без TLB работа с памятью была бы в **10-100 раз медленнее**.  
- Оптимизации (HugePages, ASID) критичны для серверов и высоконагруженных приложений.  

**Пример**:  
Сервер БД использует **HugePages**, чтобы уменьшить TLB-промахи и ускорить доступ к памяти.

---

## **22. Модель OSI. Протоколы и интерфейсы. Стек протоколов. Протокол TCP/ IP.**

### **Модель OSI (Open Systems Interconnection)**  

**Модель OSI** — это эталонная модель для описания сетевых коммуникаций, состоящая из **7 уровней**. Каждый уровень выполняет свою функцию и взаимодействует с соседними уровнями через **интерфейсы**.  
...
|----|----------------------|-------------------------------------------------------------------------|-----------------------------------|
| 7  | **Прикладной** (Application) | Интерфейс для приложений (HTTP, FTP, SMTP)                            | HTTP, FTP, SMTP, DNS              |
| 6  | **Представления** (Presentation) | Преобразование данных (шифрование, сжатие, кодирование)              | SSL/TLS, JPEG, MPEG               |
| 5  | **Сеансовый** (Session)       | Управление сеансами связи (установка, поддержка, завершение)           | RPC, SIP, NetBIOS                 |
| 4  | **Транспортный** (Transport)  | Гарантированная доставка данных (контроль ошибок, управление потоком)  | **TCP**, **UDP**, SCTP            |
| 3  | **Сетевой** (Network)        | Маршрутизация и логическая адресация (IP-адреса)                      | **IP**, ICMP, OSPF, BGP           |
| 2  | **Канальный** (Data Link)    | Передача данных между соседними узлами (MAC-адреса, контроль ошибок)  | Ethernet, Wi-Fi (802.11), PPP     |
| 1  | **Физический** (Physical)    | Передача битов по среде (кабель, радиосигнал)                         | DSL, USB, оптоволокно             |
...
#### **2.1. Протокол**  
Это набор правил, определяющих формат и порядок обмена данными между устройствами.  

**Примеры**:  
- **HTTP** (уровень 7) – передача веб-страниц.  
- **TCP** (уровень 4) – гарантированная доставка.  
- **IP** (уровень 3) – маршрутизация пакетов.  

#### **2.2. Интерфейс**  
Это точка взаимодействия между уровнями модели OSI.  

**Пример**:  
- **Сокет (Socket API)** – интерфейс между прикладным (уровень 7) и транспортным (уровень 4) уровнями.  

---

### **3. Стек протоколов TCP/IP**  

**TCP/IP** — это упрощённая и более практичная версия модели OSI, состоящая из **4 уровней**:  

| Уровень TCP/IP | Соответствие OSI | Основные протоколы                     |
|----------------|------------------|----------------------------------------|
| **Прикладной** (Application) | 5-7              | HTTP, FTP, DNS, SMTP                   |
| **Транспортный** (Transport) | 4                | **TCP** (надёжный), **UDP** (быстрый) |
| **Сетевой** (Internet)      | 3                | **IPv4/IPv6**, ICMP, ARP               |
| **Канальный** (Link)        | 1-2              | Ethernet, Wi-Fi, PPP                   |

#### **3.1. Как работает TCP/IP**  
1. **Приложение** (например, браузер) формирует HTTP-запрос.  
2. **Транспортный уровень** (TCP) разбивает данные на сегменты и добавляет порты.  
3. **Сетевой уровень** (IP) добавляет IP-адреса и маршрутизирует пакеты.  
4. **Канальный уровень** (Ethernet) передаёт кадры через физическую среду.  

---

### **4. Ключевые протоколы TCP/IP**  

#### **4.1. IP (Internet Protocol)**  
- **Задача**: Доставка пакетов от отправителя к получателю.  
- **Особенности**:  
  - **Без установки соединения** (ненадёжный, может терять пакеты).  
  - **IPv4** (32 бита, например, `192.168.1.1`).  
  - **IPv6** (128 бит, например, `2001:0db8:85a3::8a2e:0370:7334`).  

#### **4.2. TCP (Transmission Control Protocol)**  
- **Задача**: Надёжная доставка данных с контролем ошибок.  
- **Особенности**:  
  - **Установка соединения** (3-way handshake: SYN → SYN-ACK → ACK).  
  - **Подтверждение получения** (ACK).  
  - **Управление потоком** (окно перегрузки).  

**Пример TCP-сегмента**:  
```
| Source Port | Dest Port | Sequence Number | ACK Number | Flags (SYN/ACK) | Checksum | Data |
```

#### **4.3. UDP (User Datagram Protocol)**  
- **Задача**: Быстрая, но ненадёжная передача данных.  
- **Особенности**:  
  - **Нет установки соединения**.  
  - **Нет подтверждения доставки**.  
  - **Используется в VoIP, видеостриминге, DNS**.  
...
| Параметр          | TCP                          | UDP                          |
|-------------------|------------------------------|------------------------------|
| **Надёжность**    | Гарантированная доставка     | Возможна потеря данных       |
| **Скорость**      | Медленнее (из-за накладных)  | Быстрее                      |
| **Установка соединения** | Требуется (3-way handshake) | Не требуется                 |
| **Использование** | Веб-страницы (HTTP), почта (SMTP) | Видеозвонки (Zoom), игры (Quake) |

---

### **6. Пример работы стека TCP/IP**  

**Запрос сайта `example.com`**:  
1. **DNS-запрос** (UDP, порт 53) → преобразует домен в IP.  
2. **TCP-соединение** (3-way handshake) с `93.184.216.34`.  
3. **HTTP-запрос** (TCP, порт 80): `GET / HTTP/1.1`.  
4. **HTTP-ответ**: HTML-страница.  

---

### **7. Вывод**  
- **Модель OSI** — теоретическая, **TCP/IP** — практическая.  
- **TCP** — для надёжности (веб, почта), **UDP** — для скорости (стримы, игры).  
- **IP** — основа интернета, но без гарантий доставки.  
- **Интерфейсы** (например, сокеты) связывают уровни.  

**Итоговая схема**:  
```
Приложение → TCP/UDP → IP → Ethernet → Физическая среда
```
